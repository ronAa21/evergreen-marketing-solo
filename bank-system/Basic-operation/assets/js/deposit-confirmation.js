...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const completeBtn = document.querySelector(".deposit-complete-btn");
  if (completeBtn) {
  ...
  const backBtn = document.querySelector(
    ".deposit-back-btn, .receipt-back-btn"
  );
  if (backBtn) {
...
function populateTransactionInfo(data) {
  ...
  document.getElementById("deposit-ref").textContent =
    data.transaction_reference || "N/A";
  document.getElementById("deposit-date").textContent =
    data.transaction_date || "N/A";
  document.getElementById("deposit-reference").textContent =
    data.transaction_reference || "N/A";

  // Account Details
  document.getElementById("deposit-customer-name").textContent =
    data.customer_name || "N/A";
  document.getElementById("deposit-account-number").textContent =
    data.account_number ? "****" + data.account_number.slice(-4) : "N/A";
  document.getElementById("deposit-account-type").textContent =
    data.account_type || "Savings";

  // Transaction Details (right column)
  document.getElementById("deposit-amount").textContent = data.amount
    ? "PHP " + data.amount
    : "N/A";
  document.getElementById("deposit-previous-balance").textContent =
    data.previous_balance ? "PHP " + data.previous_balance : "N/A";
  document.getElementById("deposit-new-balance").textContent = data.new_balance
    ? "PHP " + data.new_balance
    : "N/A";

  // Processing Information
  document.getElementById("deposit-employee").textContent =
